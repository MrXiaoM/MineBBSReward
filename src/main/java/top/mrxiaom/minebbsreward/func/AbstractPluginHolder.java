package top.mrxiaom.minebbsreward.func;
...
import top.mrxiaom.minebbsreward.utils.ColorHelper;
...
public abstract class AbstractPluginHolder {
    ...
    public final MineBBSReward plugin;

    public AbstractPluginHolder(MineBBSReward plugin) {
        this.plugin = plugin;
    }

    public static void loadModules(MineBBSReward plugin) {
    ...
    public static void receiveFromBungee(String subChannel, byte[] bytes) {
        for (AbstractPluginHolder holder : registeredBungeeHolders.values()) {
            try (DataInputStream msgIn = new DataInputStream(new ByteArrayInputStream(bytes))) {
                ...
                MineBBSReward.getInstance().getLogger().warning(stackTraceToString(t));
            }
        }
    }
